					...
					for(const downlinkParameter of Object.values(this.adapter.downlinkConfighandler.activeDownlinkConfigs[downlinkDevice].downlinkState)){
						...
						if(downlinkDevice.length > foundLength[downlinkParameter.name]){
							...
							if(this.assignhandler.assign[downlinkParameter.name] && !options?.dontAssign){
								this.adapter.log.debug(`the state with the id ${stateId} will be assigned by internal assign function`);
								this.assignhandler.executeAssign(stateId,false,this.assignhandler.assign[downlinkParameter.name],{common:common});
							}
	...
	async setCustomObjectAtStartup(){
		...
		try{
			if(this.adapter.config.customDecodedJsonActive){
				...
				for(const adapterObject of Object.values(adapterObjectsAtStart)){
					if((adapterObject._id.indexOf(`${this.directoryhandler.reachableSubfolders.uplinkDecoded}.`) !== -1) ||
						this.adapter.config.customControlJsonActive && adapterObject._id.indexOf(`${this.directoryhandler.reachableSubfolders.downlinkControl}.`) !== -1 ||
						this.adapter.config.customConfigurationJsonActive && adapterObject._id.indexOf(`${this.directoryhandler.reachableSubfolders.configuration}.`) !== -1){
