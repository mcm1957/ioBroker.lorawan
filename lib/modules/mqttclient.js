...
class mqttClientClass {
	constructor(adapter, connectionSettings) {
		this.adapter = adapter;
		this.mqttprefix = connectionSettings.ssl ? "mqtts://" : "mqtt://";
		this.client = mqtt.connect(`${this.mqttprefix}${connectionSettings.ipUrl}`, {
	...
	async publish(topic, message, opt) {
		this.adapter.log.debug(`Publishing topic: ${topic} with message: ${message}. OPT: ${JSON.stringify(opt)}`);
