...
class mqttClientClass {
	constructor(adapter, settings) {
		...
		this.client.on("connect", () => {
			...
			const subscibtionArray = this.getSubscribtionArray();
			this.adapter.log.debug(subscibtionArray);
			// @ts-ignore +/+/devices/+/up
		...
		this.client.on("message", async (topic, message) => {
			this.adapter.log.debug(`incomming topic: ${topic} --- Message: ${message}`);
			// @ts-ignore
