...
class directorieshandlerClass {
	constructor(adapter) {
		...
		this.executeExtendedAssign = (id,assigndata,options)=>{
			...
			try{
				...
				if(!assigndata.approvedFolders || assigndata.approvedFolders[subfolder]){
					assigndata.assignfunction(assigndata,options.common);
				}
			}
			catch(error){
		...
		this.commonAssign = (assigndata,common)=>{
			for(const element in assigndata.common){
				common[element] = assigndata.common[element];
			}
		};

		this.extendedAssign = {
			DOOR_CLOSE_STATUS: {assignfunction: this.commonAssign, approvedFolders:{"uplink.decoded":true}, comon: {role:"sensor.door"}},
		};
					...
					if(!this.ignoredElementNames[elementName] && (!options || !options.ignoredElementNames || !options.ignoredElementNames[elementName])){
						...
						if(typeof obj[elementName] === "object" && obj[elementName] && !obj[elementName].isState){
							...
							if(this.extendedAssign[elementName] && !options?.dontAssign){
								...
								this.executeExtendedAssign(this.extendedAssign[elementName],{common:common});
							}
