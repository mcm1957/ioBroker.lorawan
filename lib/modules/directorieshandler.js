...
class directorieshandlerClass {
	constructor(adapter) {
		...
		// get units
		this.units = this.getJsonFromDirectoryfile(`${this.adapter.adapterDir}/lib/modules/uplinks/units.json`);
	...
	getJsonFromDirectoryfile(file){
		...
		this.adapter.log.silly(`the standard units will readout from json file.`);
		try{
